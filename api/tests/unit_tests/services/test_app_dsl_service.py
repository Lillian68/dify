...
class TestAppDslServiceVersionCompatibility:
    """
    Unit tests for _check_version_compatibility.

    This test suite covers:
    - Exact version match returns completed.
    - Invalid version format returns failed.
    """

    def test_check_version_compatibility(self):
        """Test DSL version mapping to import status."""
        # Arrange
        current = CURRENT_APP_DSL_VERSION
        parts = current.split(".")

        # Act & Assert
        assert _check_version_compatibility("999.999.999") == ImportStatus.PENDING
        assert _check_version_compatibility("0.0.0") == ImportStatus.COMPLETED_WITH_WARNINGS
        assert (
            _check_version_compatibility(f"{parts[0]}.{int(parts[1]) - 1}.{parts[2]}")
            == ImportStatus.COMPLETED_WITH_WARNINGS
        )
        assert _check_version_compatibility(current) == ImportStatus.COMPLETED
        assert _check_version_compatibility("invalid") == ImportStatus.FAILED

    def test_check_version_compatibility_major_version_mismatch(self, monkeypatch):
        """Test major version mismatch returns pending."""
        # Arrange
        monkeypatch.setattr("services.app_dsl_service.CURRENT_DSL_VERSION", "1.0.0")

        # Act & Assert
        assert _check_version_compatibility("0.5.0") == ImportStatus.PENDING
...
class TestAppDslServiceImportApp:
    ...
    @patch("services.app_dsl_service.ssrf_proxy")
    ...
    def test_import_app_yaml_content_success(
        self, mock_create_app, mock_draft_svc, mock_wf_svc, mock_ssrf, mock_redis, dsl_service, factory
    ):
    ...
    def test_import_app_yaml_url_success(
        ...
        # Arrange
        mock_ssrf.get.return_value.content = TEST_YAML_CONTENT.encode()
        mock_create_app.return_value = factory.create_workflow_app_mock()
        ...
        # Assert
        mock_ssrf.get.assert_called_once_with(TEST_RAW_YAML_URL.strip(), follow_redirects=True, timeout=(10, 10))
        assert result.status == ImportStatus.COMPLETED

    @patch("services.app_dsl_service.redis_client")
    @patch("services.app_dsl_service.ssrf_proxy")
    def test_import_app_invalid_yaml(self, mock_ssrf, mock_redis, dsl_service, factory):
    ...
    @patch("services.app_dsl_service.redis_client")
    @patch("services.app_dsl_service.ssrf_proxy")
    def test_import_app_url_fetch_failed(self, mock_ssrf, mock_redis, dsl_service, factory):
        """Test network error fails URL import."""
        # Arrange
        mock_ssrf.get.side_effect = Exception("Network error")
...
class TestAppDslServiceImportAppValidation:
    ...
    @patch("services.app_dsl_service.ssrf_proxy")
    @patch("services.app_dsl_service.redis_client")
    def test_import_app_yaml_url_file_too_large(self, mock_redis, mock_ssrf, dsl_service, factory):
        """Test URL import fails when content exceeds max size."""
        # Arrange
        response = Mock()
        response.content = b"a" * (10 * 1024 * 1024 + 1)
        response.raise_for_status.return_value = None
        mock_ssrf.get.return_value = response
    ...
    @patch("services.app_dsl_service.ssrf_proxy")
    @patch("services.app_dsl_service.redis_client")
    def test_import_app_yaml_url_empty_content(self, mock_redis, mock_ssrf, dsl_service, factory):
    ...
    def test_import_app_yaml_url_non_github(
        ...
        # Assert
        mock_ssrf.get.assert_called_once_with(non_github_url.strip(), follow_redirects=True, timeout=(10, 10))
        assert result.status == ImportStatus.COMPLETED
